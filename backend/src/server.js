...
import { isLLMEnabled, activeProvider, probe } from "./lib/llm.js";
...
export function createApp() {
  ...
  app.get("/api/selftest", async (_req, res) => {
    ...
    res.json({
      ...
      error: r.error,
      message: r.ok
        ? `✅ الذكاء الاصطناعي يعمل عبر ${r.provider} — الوكلاء يحللون العقود الجديدة بذكاء حقيقي.`
        : `⚠️ النداء فشل. سبب الخطأ: ${r.error}`,
    });
  });
